  ...
  const fetchActividades = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        ...
        imagen,
        creado_en,
        estudiante:estudiantes(id,nombre,correo,telefono),
      ...
      <section>
        ...
        <table className="w-full border-collapse border">
          ...
          <tbody>
            {actividades.map((act) => (
              <tr key={act.id}>
                <td className="border p-2">{act.estudiante.nombre}</td>
                <td className="border p-2">{act.curso.nombre}</td>
